...
static PS4ViewType* g_PS4ViewType = nullptr;


void BinaryNinja::InitPS4ViewType()
{
    static PS4ViewType type;
    BinaryViewType::Register(&type);
    g_PS4ViewType = &type;

    Ref<Settings> settings = Settings::Instance();
    settings->RegisterSetting("files.ps4.maxSectionHeaderCount",
        R"({
...
PS4View::PS4View(BinaryView* data, bool parseOnly): BinaryView("PS4", data->GetFile(), data), m_parseOnly(parseOnly)
{
    Elf64Header header;
    string errorMsg;
    BNEndianness endian;
    if (!g_PS4ViewType->ParseHeaders(data, m_ident, m_commonHeader, header, &m_arch, &m_plat, errorMsg, endian))
        throw ElfFormatException(errorMsg);

    CreateLogger("BinaryView");
    m_logger = CreateLogger("BinaryView.PS4View");
    ...
    m_backedByDatabase = data->GetFile()->IsBackedByDatabase("PS4");
    ...
    for (size_t i = 0; i < header.programHeaderCount; i++)
    {
        ...
        if (progHeader.type == ELF_PT_DYNAMIC) {
            progHeader.virtualAddress = progHeader.offset;
            
            m_dynamicTable = progHeader;
            m_logger->LogInfo("m_dynamicTable virtualAddress: 0x%x\n", progHeader.virtualAddress);
            m_dynamicTable.virtualAddress = m_dynamicTable.offset + 0x1000000;
        }
        if (progHeader.type == ELF_PT_TLS)
            m_tlsSegment = progHeader;
        
        if (progHeader.type == PT_SCE_DYNLIBDATA) {
            m_sceDynLibData = progHeader;
            m_logger->LogInfo("m_sceDynLibData: 0x%x\n", m_sceDynLibData.offset);
            m_logger->LogInfo("m_sceDynLibData.fileSize: 0x%x\n", m_sceDynLibData.fileSize);
            m_sceDynLibData.virtualAddress = m_sceDynLibData.offset + 0x1000000;
            AddAutoSegment(m_sceDynLibData.virtualAddress, m_sceDynLibData.fileSize, m_sceDynLibData.offset, m_sceDynLibData.fileSize, m_sceDynLibData.flags);
        }
    }
    ...
    catch (ReadException&)
    {
        ...
        m_logger->LogError("PS4 ELF section headers invalid");
    }
...
bool PS4View::ParseSymbolTableEntry(BinaryReader& reader, ElfSymbolTableEntry& entry, uint64_t sym,
...
void PS4View::GetRelocEntries(BinaryReader& reader, const vector<Elf64SectionHeader>& sections,
...
bool PS4View::Init()
{
    ...
    for (auto& i : m_programHeaders)
    {
        ...
        if (i.type == ELF_PT_DYNAMIC)
        {
            ...
            if (!foundMatch)
                AddAutoSection(".dynamic", m_dynamicTable.virtualAddress, m_dynamicTable.fileSize, ReadOnlyDataSectionSemantics, "DYNAMIC", 8, entrySize);
        }
    ...
    m_logger->LogInfo("m_elfSections.size = 0x%x\n", m_elfSections.size());
    uint64_t segmentStart = 0;
    for (size_t i = 1; i < m_elfSections.size(); i++)
    {
        m_logger->LogInfo("m_elfSections[i] = 0x%x\n", m_elfSections[i].type);
        string type;
    ...
    Ref<Platform> platform = Platform::GetByName("freebsd-x86_64");
    ...
    try
    {
        ...
        size_t i = 0;
        bool end = false;
        uint64_t numEntries = m_dynamicTable.fileSize / entrySize;
        m_logger->LogInfo("numEntries: 0x%x\n", numEntries);
        ...
        while (!end)
        {
            ...
            case DT_SCE_RELASZ:
                reloca.size = value;
                m_logger->LogInfo("DT_SCE_RELASZ: 0x%x\n", reloca.size);
                break;
            case DT_SCE_PLTGOT:
                gotStart = value + 0x4000;
                m_logger->LogInfo("DT_SCE_PLTGOT: 0x%x\n", gotStart);
                break;
            case DT_SCE_PLTRELSZ:
                plt.size = value;
                m_logger->LogInfo("DT_SCE_PLTRELSZ: 0x%x\n", plt.size);
                break;
            case DT_SCE_PLTREL:
                pltType = value;
    ...
    catch (ReadException&)
    {
        ...
        m_logger->LogError("PS4 ELF symbol table invalid");
    }
    ...
    try
    {
        ...
        if (m_hashtable.offset)
        {
            virtualReader.Seek(m_hashtable.offset);
            ...
            DefineDataVariable(m_hashtable.offset, Type::NamedType(this, hashTableTypeName));
            DefineAutoSymbol(new Symbol(DataSymbol, "__elf_hash_table", m_hashtable.offset, NoBinding));
    ...
    catch (ReadException&)
    {
        ...
        m_logger->LogError("PS4 ELF hash/symbol table parsing failed");
    }
    ...
    for (auto entry = combinedSymbolTable.begin(); entry != combinedSymbolTable.end(); entry++)
    {
        ...
        else
        {
            ...
            default:
            unknownType:
                m_logger->LogDebug("PS4 ELF symbol type of %d not handled.", entry->type);
                break;
            }
        }
    }
    ...
    if (m_gotEntryLocations.size() > 0)
    {
        ...
        {
            m_logger->LogWarn(
                "PS4 ELF view did not find a .got section despite detected relocations; "
...
string PS4View::ResolveNids(const string& input) {
    auto it = NIDS.find(input.substr(0, 11)); // only use the hash
    if (it != NIDS.end()) {
        return it->second;
    } else {
        // nid not found, just return original hash
        m_logger->LogInfo("NIDS Failed to resolve: %s\n", input.c_str());
        return input;
    }
}

void PS4View::DefineElfSymbol(BNSymbolType type, const string& incomingName, uint64_t addr, bool gotEntry,
    BNSymbolBinding binding, size_t size, Ref<Type> typeObj)
{
    ...
    string name = ResolveNids(incomingName);
    ...
    auto process = [=]() {
        ...
        string rawName = name;
        m_logger->LogInfo("extern symbol: %s\n", rawName.c_str());
...
void PS4View::ApplyTypesToParentStringTable(const Elf64SectionHeader& section, const bool offset)
{
...
void PS4View::ApplyTypesToStringTable(const Elf64SectionHeader& section, const int64_t imageBaseAdjustment, const bool offset)
{
...
void HexdumpStringTableCache(const std::map<uint64_t, std::vector<char>>& stringTableCache, Logger* m_logger)
{
    for (const auto& entry : stringTableCache)
    {
        uint64_t offset = entry.first;
        const std::vector<char>& data = entry.second;
        
        std::ostringstream oss;
        oss << "Offset: 0x" << std::hex << offset << std::dec;
        
        for (size_t i = 0; i < data.size(); ++i)
        {
            if (i % 16 == 0)
                oss << std::endl << std::setw(8) << std::setfill('0') << i << ": ";
            
            oss << std::setw(2) << std::setfill('0') << std::hex << (static_cast<unsigned int>(data[i]) & 0xFF) << " ";
        }
        
        oss << std::dec;
        m_logger->LogInfo("%s", oss.str().c_str());
    }
}

string PS4View::ReadStringTable(BinaryReader& reader, const Elf64SectionHeader& section, uint64_t offset)
{
    ...
    if (itr == m_stringTableCache.end())
    {
        ...
        m_logger->LogInfo("section.offset = 0x%x section.size = 0x%x\n", section.offset, section.size);
        itr = m_stringTableCache.find(section.offset);
    }

    //HexdumpStringTableCache(m_stringTableCache, m_logger);
    const std::vector<char>& tableCache = itr->second;
    return std::string(&tableCache[offset], strlen(tableCache.data() + offset));
}

vector<ElfSymbolTableEntry> PS4View::ParseSymbolTable(BinaryReader& reader, const Elf64SectionHeader& symbolSection,
...
uint64_t PS4View::PerformGetEntryPoint() const
{
    return m_entryPoint;
}


BNEndianness PS4View::PerformGetDefaultEndianness() const
{
    return m_endian;
}


bool PS4View::PerformIsRelocatable() const
{
    return m_relocatable;
}


size_t PS4View::PerformGetAddressSize() const
{
    return m_addressSize;
}


PS4ViewType::PS4ViewType(): BinaryViewType("PS4", "PS4")
{
    m_logger = LogRegistry::CreateLogger("BinaryView.PS4ViewType");
}


Ref<BinaryView> PS4ViewType::Create(BinaryView* data)
{
    try
    {
        return new PS4View(data);
    }
...
Ref<BinaryView> PS4ViewType::Parse(BinaryView* data)
{
    try
    {
        return new PS4View(data, true);
    }
    catch (std::exception& e)
    {
        m_logger->LogError("%s<BinaryViewType> failed to create view! '%s'", GetName().c_str(), e.what());
...
bool PS4ViewType::IsTypeValidForData(BinaryView* data)
{
...
uint64_t PS4ViewType::ParseHeaders(BinaryView* data, ElfIdent& ident, ElfCommonHeader& commonHeader, Elf64Header& header, Ref<Architecture>* arch, Ref<Platform>* plat, string& errorMsg, BNEndianness& endianness)
{
    ...
    Ref<Platform> recognizedPlatform = g_PS4ViewType->RecognizePlatform(commonHeader.arch, endianness, data, metadata);
    ...
    else
    {
        ...
        if (arch)
        {
            *arch = g_PS4ViewType->GetArchitecture(commonHeader.arch, codeEndianness);

            if (!*arch)
            {
                /* second try with the alternative architecture identifier */
                *arch = g_PS4ViewType->GetArchitecture(altArchId, codeEndianness);
            }
        }
    }
...
Ref<Settings> PS4ViewType::GetLoadSettingsForData(BinaryView* data)
{
